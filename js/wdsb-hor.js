...
function assignDimensions () {
	if ( !$box.hasClass( 'wdqs-inline' )) {
		...
		$box.find( 'li' ).each( function(){
			...
			if ( $( this ).find( '#wdsb-service-pinterest' ).length ) {
				var iframe = $( this ).find( '#wdsb-service-pinterest' ),
					target = $( this ).find( 'div.wdsb-item' );
				target.width( iframe.children('a').outerWidth()+iframe.children('a').children('span').outerWidth() );
			}
