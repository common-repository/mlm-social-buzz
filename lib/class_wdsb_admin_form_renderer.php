...
class Wdsb_AdminFormRenderer {
	...
	function create_services_box () {
		...
		//$pinterest_layout = $this->_get_option('pinterest_layout');
		...
		$thatmlmbeat_username = trim(get_option( "thatmlmbeat_username" ));
		foreach ($services as $key => $name) {
			...
			if (is_array($name)) {
				...
				if( $key == 'follow')
					echo "<img src='" . WDSB_PLUGIN_URL . "/img/{$key}.png' style='margin-right:32px' />";
				else
					echo "<img src='" . WDSB_PLUGIN_URL . "/img/{$key}.png' width='50px' />";
				if( $key == 'beat' && empty($thatmlmbeat_username) ){
					echo "<label for='wdsb-services-{$key}'>{$name}</label>" .
					'<br />';
					echo "<label for='wdsb-skip_script-{$key}'>" .
							'<small><a href="admin.php?page=beat">' . __('Complete thatMLMbeat Settings to activate this button', 'wdsb') . '</a></small>' .
						"</label>";
				}
				/*else if($key == 'pinterest'){
					echo "<input type='checkbox' name='wdsb[services][{$key}]' value='{$key}' " .
						"id='wdsb-services-{$key}' " .
						(in_array($key, $load) ? "checked='checked'" : "") .
					"/> " .
						"<label for='wdsb-services-{$key}'>{$name}</label>" .
					'<br />';
					echo "<select name='wdsb[pinterest_layout]'>" .
						"<option value='vertical'" . ($pinterest_layout=='vertical' ? ' selected="selected"' : '') . ">Vertical</option>" .
						"<option value='horizontal'" . ($pinterest_layout=='horizontal' ? ' selected="selected"' : '') . ">Horizontal</option>" .
						"</select> " .
						'<br />';
				}*/
				else{
	...
	function create_appearance_box () {
		...
		$layout = $this->_get_option('layout');
		...
		'<br />';
		echo '<label for="wdsb-layout">' .
			__('Layout', 'wdsb') . '</label> ' .
			'<select name="wdsb[layout]">' .
			'<option value="vertical"' . ($layout=='vertical' ? ' selected="selected"' : '') . '>Vertical</option>' .
			'<option value="horizontal"' . ($layout=='horizontal' ? ' selected="selected"' : '') . '>Horizontal</option>' .
		'<br />';
	}
	...
	function create_css_box () {
		$css = stripslashes($this->_get_option('css'));
	...
	function create_display_box () {
		...
		echo '<label>' . __('Do <b>NOT</b> show MLM Social Buzz for these types:', 'wdsb') . '</label>';
		...
		echo "" .
			'<label for="show_metabox-yes">' . __('Show MLM Social Buzz metabox in editor:', 'wdsb') . '</label> ' .
	...
	function create_manual_box () {
		...
		echo '<div><small>' . 
			...
			' <code>&lt;?php echo mlm_get_social_buzz_box(); ?&gt;</code>' .
		'</small></div>';
	}
